      ...
      <div className="space-y-3 md:space-y-5">
        ...
        <motion.div {...fadeIn(0)} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground tracking-tight">Metas</h1>
              <p className="text-xs text-muted-foreground">Acompanhe seus objetivos financeiros</p>
            </div>
          </div>
        ...
        {/* KPIs */}
        <SummaryBar items={[
          { label: "Metas Ativas", value: String(goals.length), color: "primary", icon: Target },
          { label: "Progresso Geral", value: `${overallPct.toFixed(0)}%`, color: overallPct >= 80 ? "income" : overallPct >= 40 ? "pending" : "expense", icon: TrendingUp },
          { label: "Concluídas", value: String(completedCount), color: "income", icon: Trophy },
        ]} />

        {/* Search */}
        <motion.div {...slideUp(0.1)} className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Buscar metas..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 h-9 md:h-10" />
        </motion.div>
        ...
        >
          {filtered.map(g => {
            ...
            return (
              <motion.div key={g.id} variants={staggerItem}
                ...
                className={cn(
                  "rounded-xl border bg-card p-4 md:p-5 cockpit-glow",
                  isCompleted ? "border-fin-income/30" : "border-border"
                )}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <p className={cn("text-[10px]", daysLeft < 0 ? "text-fin-expense" : daysLeft < 30 ? "text-fin-pending" : "text-muted-foreground")}>
                        {daysLeft < 0 ? `${Math.abs(daysLeft)}d atrasada` : `${daysLeft}d restantes`}
                ...
                <div className="mt-3 pt-3 border-t border-border/50">
                  {addingId === g.id ? (
                    <div className="flex gap-2">
                      <CurrencyInput
                        value={addValue}
                        onValueChange={(formatted, cents) => { setAddValue(formatted); setAddCents(cents); }}
                        className="text-xs h-8 flex-1"
                      />
                      <Button size="sm" className="h-8 text-xs px-3" onClick={() => handleAddValue(g.id, g.current_amount)}>
                        Adicionar
                      </Button>
                      <Button size="sm" variant="ghost" className="h-8 text-xs px-2" onClick={() => { setAddingId(null); setAddValue(''); setAddCents(0); }}>
                        ✕
                      </Button>
                    </div>
                  ) : (
          ...
          {filtered.length === 0 && (
            <motion.div {...fadeIn(0.2)} className="col-span-full rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">Clique em "Nova Meta" para definir seus objetivos</p>
            </motion.div>
          )}
        </motion.div>
      </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-lg">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button onClick={handleSubmit} className="w-full sm:w-auto">Salvar Meta</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PageTransition>
