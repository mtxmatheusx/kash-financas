...
const QUICK_REASONS = [
  "É um princípio pessoal/religioso que não vou mudar",
  "Já estou ciente e não preciso desse lembrete",
  "Esse gasto é necessário e não pode ser reduzido",
...
export const FinancialInsights: React.FC<FinancialInsightsProps> = ({
  ...
  const submitFeedback = async (reason: string) => {
    ...
    const { error } = await supabase
      .from("user_financial_preferences")
      .insert({ user_id: user.id, preference, category: feedbackInsight.type });
    ...
    toast.success("Preferência salva! A IA respeitará isso nas próximas análises.");
    ...
    <>
          ...
          <div className="flex items-center gap-1">
            {preferences.length > 0 && (
              ...
              >
                ...
                {preferences.length} {preferences.length === 1 ? "preferência" : "preferências"}
              </Button>
            )}
            <Button
              variant="ghost"
                ...
                {preferences.map(p => (
                  <div key={p.id} className="flex items-start justify-between gap-2 text-xs">
                    ...
                    <Button
                      ...
                      className="h-5 w-5 p-0 shrink-0 text-muted-foreground hover:text-fin-expense"
                    >
                      <X className="w-3 h-3" />
                    </Button>
                  </div>
            ...
            <AnimatePresence>
              {insights.map((insight, i) => (
                <motion.div
                  ...
                  className={`group relative rounded-lg border p-3 transition-colors hover:bg-accent/30 ${
                ...
                >
                  ...
                  <button
                    ...
                    title="Dar feedback sobre este insight"
                  >
      ...
      <Dialog open={!!feedbackInsight} onOpenChange={v => { if (!v) setFeedbackInsight(null); }}>
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base">Feedback sobre o insight</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">
              Por que esse insight não é relevante para você? A IA vai respeitar sua preferência nas próximas análises.
            </p>
            ...
            <div className="space-y-1.5">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Motivos rápidos</p>
              <div className="flex flex-wrap gap-1.5">
                {QUICK_REASONS.map(reason => (
                  <button
                    key={reason}
                    onClick={() => submitFeedback(reason)}
                    className="text-xs px-2.5 py-1.5 rounded-full border border-border bg-background hover:bg-accent/50 transition-colors text-card-foreground"
                  >
                    {reason}
                  </button>
                ))}
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1.5">Ou escreva seu motivo</p>
              <Textarea
                value={feedbackText}
                onChange={e => setFeedbackText(e.target.value)}
                placeholder="Ex: O dízimo é um princípio pessoal que não vou mudar..."
                className="min-h-[60px] text-sm"
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setFeedbackInsight(null)}>Cancelar</Button>
            <Button size="sm" onClick={() => submitFeedback(feedbackText)} disabled={!feedbackText.trim()}>
              Salvar preferência
            </Button>
          </DialogFooter>
        </DialogContent>
